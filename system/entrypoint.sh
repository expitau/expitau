...
fira_code_dir="$fonts_dir/FiraCode"
fira_code_zip_url="https://github.com/ryanoasis/nerd-fonts/releases/download/v3.1.1/FiraCode.zip"

# Check if FiraCode directory exists
if [ ! -d "$fira_code_dir" ]; then
    echo "FiraCode directory not found. Installing..."

    # Check if wget and unzip are installed
    if ! command -v wget &> /dev/null || ! command -v unzip &> /dev/null; then
        echo "FiraCode install error: wget or unzip not installed"
    else
        # Download and unzip FiraCode
        echo "Downloading and installing FiraCode..."
        mkdir -p "$fonts_dir"
        wget -O /tmp/FiraCode.zip "$fira_code_zip_url"
        unzip -q /tmp/FiraCode.zip -d "$fira_code_dir"
        rm /tmp/FiraCode.zip

        fc-cache -f

        echo "FiraCode installed successfully!"
    fi
fi
